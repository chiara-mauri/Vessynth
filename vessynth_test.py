...
if __name__ == "__main__":   
    ...
    parser.add_argument('-c', '--cutout', type=str, nargs='+', default=None,
                        help='A bounding box to identify ROI "x1,x2,y1,y2,z1,z2" ')
    ...
    if modality == 'OCT':
        model_to_load = glob.glob(os.path.join(model_path, 'weights/OCT_model*'))[0]
        json_path = os.path.join(model_path, 'segnet_model_OCT.json') #json file containing backbone info
    elif modality == 'T2star':
        model_to_load = glob.glob(os.path.join(model_path, 'weights/T2star_model*'))[0]
        json_path = os.path.join(model_path, 'segnet_model_T2star.json')
    elif modality == 'TOF':
        model_to_load = glob.glob(os.path.join(model_path, 'weights/TOF_model*'))[0]
        json_path = os.path.join(model_path, 'segnet_model_TOF.json')
    elif modality == 'HiPCT':
        model_to_load = glob.glob(os.path.join(model_path, 'weights/HiPCT_model*'))[0]
        json_path = os.path.join(model_path, 'segnet_model_HiPCT.json')
    elif modality == 'fibers':
        model_to_load = glob.glob(os.path.join(model_path, 'weights/axons_model*'))[0]
        json_path = os.path.join(model_path, 'segnet_model_fibers.json')
    elif modality == 'LSFM':
        model_to_load = glob.glob(os.path.join(model_path, 'weights/LSFM_model*'))[0]
    ...
    with torch.no_grad():
        ...
        for vol_index, vol in enumerate(volumes):
            ...
            if cutout is not None:
                x1, x2, y1, y2, z1, z2 = map(int, cutout[vol_index].split(','))
                save_name += f"_cutout_{x1}_{x2}_{y1}_{y2}_{z1}_{z2}"
